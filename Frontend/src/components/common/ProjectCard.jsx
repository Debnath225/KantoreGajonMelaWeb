import { motion } from "framer-motion";
function ProjectCard() {
  const date = new Date();
  const projectsData = {
    id: "1",
    projectName: "Kantore Gajon Mela",
    year: date.getFullYear(),
    projectDescription:
      "Full-stack App to Showcase My Village Gajon Mela's Some Moments and  Help User to Know About Baba Kantorenath .",
    usedTech: [
      "React",
      "Node.js",
      "Express",
      "MongoDB",
      "TailwindCSS",
      "GSAP",
      "Famer Motion",
      "Lenis",
    ],
    githubLink: "https://github.com/Debnath225/KantoreGajonMelaWeb",
  };

  return (
    <motion.div
      initial={{ scale: 0, opacity: 0 }}
      whileInView={{ scale: 1, opacity: 1 }}
      className="h-full"
    >
      {/* <!-- Project Card 1 --> */}
      <div className="group relative w-full h-full bg-slate-950/70 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col">
        {/* <!-- Project Image --> */}
        <div className="relative aspect-[16/10] overflow-hidden">
          <img
            src="/images/AC-2026-002.webp"
            alt="Project:-Kantore Gajon Mela(Kantore2026)"
            loading="lazy"
            decoding="async"
            className="block w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-500"
          />
          {/* <!-- Overlay on Hover --> */}
          <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
            <a
              href={projectsData.githubLink}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Open project repository"
              className="p-3 bg-white/10 backdrop-blur-md rounded-full hover:bg-blue-500 transition-colors"
            >
              <svg
                className="w-6 h-6"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                ></path>
              </svg>
            </a>
            <a
              href={projectsData.githubLink}
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Open project source code"
              className="p-3 bg-white/10 backdrop-blur-md rounded-full hover:bg-blue-500 transition-colors"
            >
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.6.113.793-.261.793-.577v-2.234c-3.338.726-4.042-1.416-4.042-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 12.3 24 12c0-6.627-5.373-12-12-12z" />
              </svg>
            </a>
          </div>
        </div>

        {/* <!-- Project Content --> */}
        <div className="p-5 md:p-6 space-y-4 flex-1">
          <div className="flex justify-between items-start">
            <h4 className="section-title !text-xl text-cyan-300 transition-colors">
              {projectsData.projectName}
            </h4>
            <span className="typo-small !text-xs font-mono bg-blue-500/10 text-blue-300 px-2 py-1 rounded-md">
              {projectsData.year}
            </span>
          </div>
          <p className="typo-body text-slate-300/90 hover:text-slate-200">
            {projectsData.projectDescription}
          </p>

          {/* <!-- Tags --> */}
          {/* Tags Hading  */}
          <div className="flex">
            <h2 className="typo-h3 text-cyan-200">Used Tech</h2>
          </div>
          <div className="flex flex-wrap gap-2 pt-2">
            {projectsData.usedTech.map((tech) => {
              return (
                <span
                  key={tech}
                  className="typo-micro !text-[10px] text-cyan-200/90 border border-cyan-400/35 px-2.5 py-1 rounded-md bg-cyan-400/[0.06]"
                >
                  {tech}
                </span>
              );
            })}
          </div>
        </div>
      </div>

      {/* <!-- More Project Cards can be added here following the same structure --> */}
    </motion.div>
  );
}

export default ProjectCard;
